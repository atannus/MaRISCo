...
def Update (slice,te, tr, ti):
    ...
    else: # no PIL? - take this pill!
        # this a) slower than PIL and b) only a nearest neighbor interpolation 
...
# read GM - extract infos
try: GM_image = nib.load('GM.nii.gz');
except: 
    root.withdraw(); 
    showerror(Program_name,' Error: GM.nii.gz file not found         ');
    sys.exit(1); 
data_pixdim = GM_image.header.get_zooms();
GM_data = GM_image.get_data();
data_dim = GM_data.shape
total_slices = data_dim[2];
zoom_X=int(data_dim[0]*data_pixdim[0]*2.5); # enlarge x 2.5 (used in Update)
zoom_Y=int(data_dim[1]*data_pixdim[1]*2.5); # enlarge x 2.5 (used in Update) 
# read WM, CSF   
try: WM_data = nib.load('WM.nii.gz').get_data();
except:
    root.withdraw(); 
    showerror(Program_name,' Error: WM.nii.gz file not found         ');
    sys.exit(1); 
try: CSF_data = nib.load('CSF.nii.gz').get_data();
except:
    root.withdraw(); 
    showerror(Program_name,' Error: CSF.nii.gz file not found         ');
    sys.exit(1); 
...
# Image Definition & Initialize with 1 pixel
IMG_tkLabel=tk.Label(image=tk.PhotoImage(data="P5 1 1 255\n\x00"));
...
TE_tkScale = tk.Scale(root, command=validateTE, variable=TE_tkVar, 
    ...
    fg='black', troughcolor=ltgray, highlightcolor=sysbg, font=('', 8), 
...
SL_tkLabel  = tk.Label(root, font=('', 8), textvariable=SL_tkVar);
# Layout
# ---------------------------------------
# | Slice number |   Image   |    <     |
# | Slice slider |     ^     |    <     |
